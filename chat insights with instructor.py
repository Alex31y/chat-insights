...
class SemanticSearch:
    ...
    def fit(self, data, batch=1000, n_neighbors=5):
        ...
        embeddings = self.get_text_embedding(data, batch=batch)  # qui creo gli embedding
    ...
    def __call__(self, text): # text è la domanda input dell'utente
        query = [['Represent question for retrieving supporting documents: ', text]]
        print(query)
        top_k = min(n_chunks, self.dim_corpus)
        domanda_embeddings = self.encode(query) # embedding applicato alla domanda
        ...
        similarities = cosine_similarity(domanda_embeddings, np.array(self.corpus_embeddings))
        top_results = np.argsort(similarities)[0][-top_k:]  # Retrieve indices of top_k most similar elements
        sorted_results = sorted(top_results, key=lambda i: similarities[0][i], reverse=True)  # Sort indices by similarity in descending order
        return [self.data[i] for i in sorted_results]  # Retrieve the most similar elements in the order of similarity
    ...
    def get_text_embedding(self, texts, batch):
        ...
        for i in range(0, len(texts), batch):
            ...
            for text in text_batch:
                chunk = ['Represent the document for retrieval: ', text]
                #print(chunk)
                corpus.append(chunk)
