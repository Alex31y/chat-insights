...
embeddings_file = f"docs\embeddings.index"
...
class SemanticSearch:
    ...
    def fit2(self, embeddings_file):
        stored = faiss.read_index(embeddings_file)
        self.corpus_embeddings = stored["embeddings"]  # qui creo gli embedding
        self.dim_corpus = self.index.ntotal
        self.fitted = True
    ...
    def __call__(self, text): # text è la domanda input dell'utente
        ...
        top_k = self.index.search(domanda_embeddings, top_k)
        print('>>>> Results in Total Time: {}'.format(time.time() - t))
        top_k_ids = top_k[1].tolist()[0]
        top_k_ids = list(np.unique(top_k_ids))
        return [self.data[i] for i in top_k_ids]
    ...
    def get_text_embedding(self, texts, batch):
        ...
        #faiss
        encoded_data = np.asarray(embeddings.astype('float32'))
        self.index = faiss.IndexIDMap(faiss.IndexFlatIP(768))
        self.index.add_with_ids(encoded_data, np.array(range(0, len(texts))))
        faiss.write_index(self.index, 'embedding.index')
